...
namespace QuizMIS.Models {
    ...
    public class TeacherStoreContext {
        ...
        public  List<QuizResult> GetQuizResults(int QuizId) {
            ...
            List<QuizResult> q=new List<QuizResult>();

            MySqlCommand cmd=new MySqlCommand("select DISTINCT  s.StudentName,a.marks from answer as a inner join student as s on s.StudentId=a.StudentId AND a.QuizId=@v1",conn);
            cmd.Parameters.AddWithValue("@v1",QuizId);
            cmd.Prepare();

            var reader=cmd.ExecuteReader();
            while(reader.Read()) {
                q.Add(new QuizResult(){
        ...
        public string GetQuizTitle(int QuizId) {
            ...
            MySqlCommand cmd=new MySqlCommand("select QuizTitle from quiz where QuizId=@v1",conn);
            cmd.Parameters.AddWithValue("@v1",QuizId);
            cmd.Prepare();
